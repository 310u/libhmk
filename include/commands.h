...
typedef enum {
  ...
  COMMAND_GET_ANALOG_CONFIG,
  COMMAND_SET_ANALOG_CONFIG,
...
typedef struct __attribute__((packed)) {
  uint8_t profile;
  uint8_t offset; // Index in analog_configs array
  uint8_t len;
  analog_config_t analog_configs[4]; // Matches NUM_ANALOG_CONFIGS ideally, or chunked
} command_in_analog_config_t;
...
typedef struct __attribute__((packed)) {
  ...
  union __attribute__((packed)) {
    ...
    command_in_analog_config_t analog_config;
  };
} command_in_buffer_t;
