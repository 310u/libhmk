...
#define RGB_RESET_TIME_TICKS                                                     \
  ((uint32_t)(((uint64_t)F_CPU * 100000ULL + 999999999ULL) /                     \
              1000000000ULL))
...
void rgb_driver_write(const uint8_t *grb_data, uint16_t byte_count) {
  ...
  tmr_channel_value_set(RGB_TIMER, RGB_TIMER_CHANNEL, rgb_dma_buffer[0]);
  tmr_event_sw_trigger(RGB_TIMER, TMR_OVERFLOW_SWTRIG);

  if (total_slots > 1u) {
    RGB_DMA_CHANNEL->maddr = (uint32_t)&rgb_dma_buffer[1];
    dma_data_number_set(RGB_DMA_CHANNEL, (uint16_t)(total_slots - 1u));
