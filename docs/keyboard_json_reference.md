...
> [!IMPORTANT]
> `features` で機能を有効化した場合、対応するハードウェア定義を `board_def.h` に記述する必要があります。詳細は[`board_def.h` セクション](#board_def)を参照してください。
...
| `driver` | string | ✅ | ハードウェアドライバ名（`"at32f405xx"`, `"stm32f446xx"` など） |
...
| `matrix` | integer[][] | マルチプレクサチャンネルからキーインデックスへのマッピング |
...
| `vector` | integer[] | 各入力に対応するキーインデックス |
...
> [!TIP]
> `raw` セクションはジョイスティックのアナログ軸の入力に利用されます。`vector` の値は `keyboard.num_keys` 以上のインデックスを指定することで、通常のキーとは別にジョイスティック用の内部入力として扱えます。
...
`features` で有効化した機能に対応するハードウェア定義を `keyboards/<keyboard_name>/board_def.h` に記述します。
...
#define JOYSTICK_SW_KEY_INDEX 40         // スイッチに対応するキーインデックス
```
...
```c
#define SLIDER_KEY_INDEX 39              // スライダーに対応するキーインデックス
```
